...
def employees_route(uri, template, dbHndlr):
    @route(uri, name=uri)
    class employees_handler(tornado.web.RequestHandler):
        
        _dbHndlr=dbHndlr
        _dtbs=TmpltDtbs(_dbHndlr,["users"])
        _template = template

        def get(self):
            if not current_user(self)["UserID"] or current_user(self)["UserType"] != "admin":
                self.redirect("/")
            else:
